...
def create_scoreboard(hits, scale, ringsAmount, innerDiam):
    ...
    for hit in hits:
        ...
        hit_obj = Hit(int(hit[0]), int(hit[1]), score, hit[3]) # 创建撞击点
...
def get_hit(group, point, distanceTolerance):
    ...
    hits_list = get_hits(group)  # 0 [](初始时) #第二次 hit
    compatible_hits = []

    for hit in hits_list:
        if geo2D.euclidean_dist(point, hit.point) <= distanceTolerance:
            compatible_hits.append(hit)  # 将另一点视为同一点
            
    if len(compatible_hits) > 0:
        return compatible_hits[0]   # 返回第一个点
    else:
        return None
...
def sort_hit(hit, distanceTolerance, minVerifiedReputation):
    ...
    if type(candidate) != type(None):
        candidate.increase_rep()    # reputation+1
        candidate.iter_mark = True  #下一个周期检测到点添加到candidate
        ...
        if candidate.isVerified(minVerifiedReputation):
            ...
            # find duplicate verified hits and eliminate them
            eliminate_verified_redundancy(distanceTolerance)    # 命中点加入varified

    # new candidate
    else:
        candidate_hits.append(hit)  # 加入hit的点
        hit.iter_mark = True    #?
...
def get_hits(group):
    ...
    return switcher.get(group, [])  # candidate_hits