...
class VideoAnalyzer:
    ...
    def _analyze_frame(self, frame):
        ...
        # 这一步的目的是获得靶子图片与frame之间的特征点matches
        ...
        if len(matches) >= 4:
            ...
            if type(homography) != type(None):
                warped_transform = cv2.perspectiveTransform(self.anchor_points, homography)  # 找框

                # x = cv2.polylines(frame, [np.int32(warped_transform)], True, 255, 3, cv2.LINE_AA)
                # cv2.namedWindow('4', 0)
                # cv2.imshow("4", x)
                # cv2.waitKey()
                ...
                if matcher.is_true_homography(warped_vertices, warped_edges, (self.frame_w, self.frame_h), .2):
                    ...
                    warped_img = cv2.warpPerspective(self.pad_model, homography, (self.frame_w, self.frame_h))  # 对应变换
                    cv2.imwrite("out.jpg", warped_img)
                    scale = geo2D.calc_model_scale(warped_edges, self.model.shape)  # 计算翘曲单应变换的相对比例到实际模型的形状。
                    
                    # process image
                    sub_target = visuals.subtract_background(warped_img, frame)     # 和原始帧的差别
                    pixel_distances = geo2D.calc_distances_from(frame.shape, warped_vertices[5])    # 计算圆心和页面的距离（类似热力图？）
                    estimated_warped_radius = self.rings_amount * self.inner_diam * scale[2]    # 50*6*1.
                    circle_radius, emphasized_lines = visuals.emphasize_lines(sub_target, pixel_distances,
                                                                    estimated_warped_radius)    # 画圆 二值化和连线
                    print(warped_vertices)
                    
                    proj_contours = visuals.reproduce_proj_contours(emphasized_lines, pixel_distances,
                                                                    warped_vertices[5], circle_radius)  # 边缘检测

                    suspect_hits = visuals.find_suspect_hits(proj_contours, warped_vertices, scale)  # 找到真正的命中值
                    # （x,y,d,圆心）
        ...
        while True:
            ...
            if ret:
                ...
                for hit in scoreboard:
                    hitsMngr.sort_hit(hit, 30, 15)  # 所有命中点
                ...
                grouping_contour = grouper.create_group_polygon(frame, verified_hits)   # 画多边形
