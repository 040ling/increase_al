...
class CameraManager(object):
    ...
    def start(self,idx,DisplayMode,path,save_path,model,bullseye,innerdist,num_target,score):
        # 开始工作
        tool.PRINT("开启视频采集")
        if self.DisplayMode:
            cam = CAM.cam(idx, DisplayMode, path, save_path, model,bullseye,innerdist,num_target)
            frameThread = threading.Thread(target=self.worker2, args=(cam,score))
            frameThread.start()
        else:
            cam = CAM.cam(idx, DisplayMode, path, save_path, model, bullseye, innerdist, num_target)
            frameThread = threading.Thread(target=self.worker, args=(cam,score))
            frameThread.start()
        return self

    def worker(self, ele, score):
        print('3214')
        while self._isWorking:
            start = time.time()
            # try:
            ele.cam_detect(score)
            while time.time() - start < 0.08:
                time.sleep(0.001)

    def worker2(self, ele, score):
        print('12374')
        idx = 0
        while self._isWorking:
            start = time.time()
            ...
            if idx == 15:
                ...
                idx += 1
            normal = ele.video_detect(idx, score)
...
if __name__ == "__main__":
    ...
    camera.start(0,DisplayMode,path,save_path,model,bullseye,innerdist,num_target,20)
    """
    while True:
